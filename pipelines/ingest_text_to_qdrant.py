...
from knowledgebase_data.text_file_parser import parse_text_file
...
class TextFileIngestionPipeline:
    ...
    def _parse_text_file(self):
        """Parse the text file into blocks with extracted metadata."""
        logger.info("Parsing text file: %s", self.text_path)
        logger.info("Each block separated by '-----------------------------' will be treated as a full chunk.")
        self.records = parse_text_file(str(self.text_path))
        logger.info("✅ Prepared %d text blocks (chunks).", len(self.records))
        if self.records:
            logger.info(
                "Sample chunk ID: %s (City: %s, Aspect: %s, Sub-Aspect: %s)",
                self.records[0]["id"],
                self.records[0]["city"],
                self.records[0]["aspect"],
                self.records[0]["subaspect"],
            )

    def _embed_blocks(self):
        """Embed all text blocks in batches."""
        logger.info(
    ...
    def _upsert_blocks(self):
        ...
        metadatas = [
            {
                ...
                "source_file": self.text_path.name,
            }
        ...
        logger.info(
            "✅ Uploaded %d chunks (records) to Qdrant collection '%s'.",
            len(self.records),
            self.collection_name,
        )
    ...
    def _verify_search(self, limit: int = 3):
        ...
        try:
            # Use the first record's text and use batch_embed to ensure consistent dimensions
            sample_text = self.records[0]["text"]
            logger.info("Running verification search against Qdrant (top %d).", limit)
            logger.info("Re-embedding sample text for verification using batch_embed for consistency...")

            # Use batch_embed with single item to ensure same dimension as upload
            verification_vectors = self.openai_client.batch_embed([sample_text])
            if not verification_vectors or len(verification_vectors) == 0:
                logger.warning("Failed to create verification embedding. Skipping search.")
                return

            verification_vector = verification_vectors[0]
            ...
            logger.info(
                "Verification vector dimension: %d (expected: %d)",
                len(verification_vector),
                self.embedding_size,
            )
    ...
    def run(self):
        ...
        try:
            ...
            if not self.records:
                logger.warning("No records found in %s. Nothing to do.", self.text_path)
                return
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    ...
    default_collection = (
        settings.RPF_KB_COLLECTION_NAME if hasattr(settings, "RPF_KB_COLLECTION_NAME") else "rpf_kb"
    )

    parser = argparse.ArgumentParser(
        description="Parse text file into blocks and ingest into Qdrant knowledge base."
    )
    parser.add_argument(
        "--text-path",
        type=Path,
        default=default_text,
        help="Path to the text file to parse and ingest.",
    )
    parser.add_argument(
        "--collection-name",
        type=str,
        default=default_collection,
        help="Qdrant collection name (default: from config RPF_KB_COLLECTION_NAME).",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        ...
        default=True,
        help="Do NOT reset the collection (append to existing collection). Default is to reset.",
    )
